// =========================================================================
// ⚙️ CẤU HÌNH API LINH HOẠT CHO WEB & ANDROID APP
function resolveApiBase() {
    const saved = localStorage.getItem('pos_custom_api_url');
    if (saved && saved.trim()) {
        let clean = saved.trim();
        if (clean.endsWith('/')) clean = clean.slice(0, -1);
        if (!clean.endsWith('/api') && !clean.includes('/api/')) clean += '/api';
        return clean;
    }
    if (window.location.protocol === 'file:' || window.location.protocol === 'capacitor:') {
        return 'https://giavdn.pro.vn/api';
    }
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        return (window.location.port === '3000' || window.location.port === '') ? '/api' : 'http://localhost:3000/api';
    }
...
let db = createApiClient(API_BASE);

// Các hàm cấu hình kết nối máy chủ cho App Android & Web
function openServerConfigModal() {
    const current = localStorage.getItem('pos_custom_api_url') || API_BASE;
...
function saveServerConfig() {
    ...
    if (url) {
        ...
        localStorage.setItem('pos_custom_api_url', url);
    } else {
        localStorage.removeItem('pos_custom_api_url');
    }
    API_BASE = resolveApiBase();
    db = createApiClient(API_BASE);
    alert('Đã cập nhật máy chủ: ' + API_BASE);
...
async function testServerConnection() {
    ...
    let url = input.value.trim() || 'https://giavdn.pro.vn/api';
    if (!url.startsWith('http://') && !url.startsWith('https://')) {
        url = 'http://' + url;
    }
    if (url.endsWith('/')) url = url.slice(0, -1);
    ...
    try {
        ...
        const res = await fetch(`${url}/users`, { signal: controller.signal });
        clearTimeout(timeoutId);
        if (res.ok) {
            statusEl.innerHTML = '✅ Kết nối máy chủ thành công!';
            statusEl.style.color = '#27ae60';
        } else {
            statusEl.innerHTML = `⚠️ Máy chủ phản hồi mã lỗi: ${res.status}`;
            statusEl.style.color = '#e67e22';
        }
    } catch (e) {
        statusEl.innerHTML = `❌ Không thể kết nối: ${e.message}`;
...
window.onAndroidBackPressed = function() {
    ...
    const menuBox = document.getElementById('menuManagerBox');
    if (menuBox && menuBox.style.display !== 'none') {
        if (typeof cancelEditMenu === 'function') cancelEditMenu();
        return true;
    }
    const storeSettings = document.getElementById('storeSettingsBox');
    if (storeSettings && storeSettings.style.display !== 'none') {
        if (typeof toggleStoreSettings === 'function') toggleStoreSettings();
        return true;
    }
    return false;
};
